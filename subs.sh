...
echo $domain
 if [[ $long != "false" ]]; then 
     ...
     ; bbot -t $domain -f subdomain-enum -rf passive -c modules.massdns.max_resolvers=1000 --output-module json --yes -s 2>/dev/null | jq -r 'select(.type=="DNS_NAME") | .data' || >&2 echo -e "$Cross bbot failed"
     )
